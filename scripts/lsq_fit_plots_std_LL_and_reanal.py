# -*- coding: iso-8859-1 -*-
'''
run_lsq_fit_plots_std_LL_and_reanal  uses  lsq_fit_read and lsq_fit_plots

Date: 8 March 2023
Purpose: Plot out dependence of least squares fits on time-averaging    ; plots std for both ak306 (LL) and reanalysis
Author: Mike Bell   
...
# 1. build file names - most of this is copied from timeseries_lsq_fit_multiple.py 
 ...
 for iexpt in [0,1] : 
   ...
   if iexpt == 0 : 
      ...
      expt_id1 = c_exp ; expt_id2 = expt_id1 ; expt_id3 = expt_id1
...
#    plt.plot(time_axis,v_seq*1.E-4,color='k',linewidth=2, linestyle='-')   factor 1.E-4 removed from following 3 lines (24/04/2019)
   ...
   for item in [0, 1, 2, 3] : 
       ...
       col     =  col_type[item] 
       vals = values[item]        
       plt.plot(list_len_tim_avg,vals, color=col, linewidth=8, linestyle=lstyle, label = plt_lab)
...
#   plt.legend()     # there are multiple legends for each line (because it is plotted for each segment so labels & legends are commented out 
   ...
   if iexpt == 0 : 
      plotfilename = infolder + c_exp + '_' + file_nam_in +'_'+ c_fit_var + '.png'
